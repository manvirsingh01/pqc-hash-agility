...
echo "└──────────────┴───────────────────┴──────────────────────────────────────┘"
echo ""
echo "All backends compiled at matched optimisation tier (no -march=native)."
echo "SHAKE uses liboqs scalar Keccak (OQS_DIST_BUILD=OFF, OQS_USE_OPENSSL=OFF)."
...
echo ""
echo "Compiler flags : -O3 (all backends, no -march=native for parity)"
