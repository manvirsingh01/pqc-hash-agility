...
/*
 * PQCLEAN_MLDSA87_HARAKA_dilithium_stream128_init / stream256_init
...
void PQCLEAN_MLDSA87_HARAKA_dilithium_stream128_init(stream128_state *state, const uint8_t seed[SEEDBYTES], uint16_t nonce) {
    ...
    PQCLEAN_MLDSA87_HARAKA_xof_init(state);
    PQCLEAN_MLDSA87_HARAKA_xof_absorb(state, seed, SEEDBYTES);
    PQCLEAN_MLDSA87_HARAKA_xof_absorb(state, t, 2);
    PQCLEAN_MLDSA87_HARAKA_xof_finalize(state, XOF_DOMAIN_MATRIX);
}
...
void PQCLEAN_MLDSA87_HARAKA_dilithium_stream256_init(stream256_state *state, const uint8_t seed[CRHBYTES], uint16_t nonce) {
    ...
    PQCLEAN_MLDSA87_HARAKA_xof_init(state);
    PQCLEAN_MLDSA87_HARAKA_xof_absorb(state, seed, CRHBYTES);
    PQCLEAN_MLDSA87_HARAKA_xof_absorb(state, t, 2);
    PQCLEAN_MLDSA87_HARAKA_xof_finalize(state, XOF_DOMAIN_NOISE);
}

void PQCLEAN_MLDSA87_HARAKA_dilithium_squeezeblocks(uint8_t *out, size_t nblocks, xof_state *state) {
    PQCLEAN_MLDSA87_HARAKA_xof_squeeze(out, nblocks * XOF_BLOCKBYTES, state);
}
