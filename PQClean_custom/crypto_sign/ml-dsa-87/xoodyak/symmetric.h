#ifndef PQCLEAN_MLDSA87_XOODYAK_SYMMETRIC_H
#define PQCLEAN_MLDSA87_XOODYAK_SYMMETRIC_H
#include "params.h"
#include <stddef.h>
#include <stdint.h>
...
void PQCLEAN_MLDSA87_XOODYAK_dilithium_stream128_init(stream128_state *state, const uint8_t seed[SEEDBYTES], uint16_t nonce);
void PQCLEAN_MLDSA87_XOODYAK_dilithium_stream256_init(stream256_state *state, const uint8_t seed[CRHBYTES], uint16_t nonce);
void PQCLEAN_MLDSA87_XOODYAK_dilithium_squeezeblocks(uint8_t *out, size_t nblocks, xof_state *state);

void PQCLEAN_MLDSA87_XOODYAK_xof_init(xof_state *state);
void PQCLEAN_MLDSA87_XOODYAK_xof_absorb(xof_state *state, const uint8_t *in, size_t inlen);
void PQCLEAN_MLDSA87_XOODYAK_xof_finalize(xof_state *state, uint8_t domain);
void PQCLEAN_MLDSA87_XOODYAK_xof_squeeze(uint8_t *out, size_t outlen, xof_state *state);

#define stream128_init(STATE, SEED, NONCE) PQCLEAN_MLDSA87_XOODYAK_dilithium_stream128_init(STATE, SEED, NONCE)
#define stream128_squeezeblocks(OUT, OUTBLOCKS, STATE) PQCLEAN_MLDSA87_XOODYAK_dilithium_squeezeblocks(OUT, OUTBLOCKS, STATE)
#define stream128_release(STATE) ((void)(STATE))

#define stream256_init(STATE, SEED, NONCE) PQCLEAN_MLDSA87_XOODYAK_dilithium_stream256_init(STATE, SEED, NONCE)
#define stream256_squeezeblocks(OUT, OUTBLOCKS, STATE) PQCLEAN_MLDSA87_XOODYAK_dilithium_squeezeblocks(OUT, OUTBLOCKS, STATE)
#define stream256_release(STATE) ((void)(STATE))

#define xof_init(STATE) PQCLEAN_MLDSA87_XOODYAK_xof_init(STATE)
#define xof_absorb(STATE, IN, INLEN) PQCLEAN_MLDSA87_XOODYAK_xof_absorb(STATE, IN, INLEN)
#define xof_finalize(STATE, DOMAIN) PQCLEAN_MLDSA87_XOODYAK_xof_finalize(STATE, DOMAIN)
#define xof_squeeze(OUT, OUTLEN, STATE) PQCLEAN_MLDSA87_XOODYAK_xof_squeeze(OUT, OUTLEN, STATE)
