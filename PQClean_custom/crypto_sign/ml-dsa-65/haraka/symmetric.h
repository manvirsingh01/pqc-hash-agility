#ifndef PQCLEAN_MLDSA65_HARAKA_SYMMETRIC_H
#define PQCLEAN_MLDSA65_HARAKA_SYMMETRIC_H
#include "params.h"
#include <stddef.h>
#include <stdint.h>
...
#define stream128_init(STATE, SEED, NONCE) PQCLEAN_MLDSA65_HARAKA_dilithium_stream128_init(STATE, SEED, NONCE)
#define stream128_squeezeblocks(OUT, OUTBLOCKS, STATE) PQCLEAN_MLDSA65_HARAKA_dilithium_squeezeblocks(OUT, OUTBLOCKS, STATE)
#define stream128_release(STATE) ((void)(STATE))

#define stream256_init(STATE, SEED, NONCE) PQCLEAN_MLDSA65_HARAKA_dilithium_stream256_init(STATE, SEED, NONCE)
#define stream256_squeezeblocks(OUT, OUTBLOCKS, STATE) PQCLEAN_MLDSA65_HARAKA_dilithium_squeezeblocks(OUT, OUTBLOCKS, STATE)
#define stream256_release(STATE) ((void)(STATE))

#define xof_init(STATE) PQCLEAN_MLDSA65_HARAKA_xof_init(STATE)
#define xof_absorb(STATE, IN, INLEN) PQCLEAN_MLDSA65_HARAKA_xof_absorb(STATE, IN, INLEN)
#define xof_finalize(STATE, DOMAIN) PQCLEAN_MLDSA65_HARAKA_xof_finalize(STATE, DOMAIN)
#define xof_squeeze(OUT, OUTLEN, STATE) PQCLEAN_MLDSA65_HARAKA_xof_squeeze(OUT, OUTLEN, STATE)
