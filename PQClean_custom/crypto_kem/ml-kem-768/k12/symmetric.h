#ifndef PQCLEAN_MLKEM768_K12_SYMMETRIC_H
#define PQCLEAN_MLKEM768_K12_SYMMETRIC_H
#include "fips202.h"
#include "params.h"
...
void PQCLEAN_MLKEM768_K12_kyber_k12_absorb(xof_state *s,
        const uint8_t seed[KYBER_SYMBYTES],
        uint8_t x,
        uint8_t y);

void PQCLEAN_MLKEM768_K12_kyber_k12_squeezeblocks(uint8_t *out, size_t nblocks, xof_state *s);

void PQCLEAN_MLKEM768_K12_kyber_k12_ctx_release(xof_state *s);

void PQCLEAN_MLKEM768_K12_kyber_k12_prf(uint8_t *out, size_t outlen, const uint8_t key[KYBER_SYMBYTES], uint8_t nonce);

void PQCLEAN_MLKEM768_K12_kyber_k12_rkprf(uint8_t out[KYBER_SSBYTES], const uint8_t key[KYBER_SYMBYTES], const uint8_t input[KYBER_CIPHERTEXTBYTES]);
...
#define xof_absorb(STATE, SEED, X, Y) PQCLEAN_MLKEM768_K12_kyber_k12_absorb(STATE, SEED, X, Y)
#define xof_squeezeblocks(OUT, OUTBLOCKS, STATE) PQCLEAN_MLKEM768_K12_kyber_k12_squeezeblocks(OUT, OUTBLOCKS, STATE)
#define xof_ctx_release(STATE) PQCLEAN_MLKEM768_K12_kyber_k12_ctx_release(STATE)
#define prf(OUT, OUTBYTES, KEY, NONCE) PQCLEAN_MLKEM768_K12_kyber_k12_prf(OUT, OUTBYTES, KEY, NONCE)
#define rkprf(OUT, KEY, INPUT) PQCLEAN_MLKEM768_K12_kyber_k12_rkprf(OUT, KEY, INPUT)
