#ifndef PQCLEAN_MLKEM768_XOODYAK_SYMMETRIC_H
#define PQCLEAN_MLKEM768_XOODYAK_SYMMETRIC_H
#include "Xoodyak.h"
#include "fips202.h"
#include "params.h"
#include <stddef.h>
...
void PQCLEAN_MLKEM768_XOODYAK_kyber_xoodyak_absorb(xof_state *s,
        const uint8_t seed[KYBER_SYMBYTES],
        uint8_t x,
        uint8_t y);

void PQCLEAN_MLKEM768_XOODYAK_kyber_xoodyak_squeezeblocks(uint8_t *out, size_t nblocks, xof_state *s);

void PQCLEAN_MLKEM768_XOODYAK_kyber_xoodyak_ctx_release(xof_state *s);
...
#define xof_absorb(STATE, SEED, X, Y) PQCLEAN_MLKEM768_XOODYAK_kyber_xoodyak_absorb(STATE, SEED, X, Y)
#define xof_squeezeblocks(OUT, OUTBLOCKS, STATE) PQCLEAN_MLKEM768_XOODYAK_kyber_xoodyak_squeezeblocks(OUT, OUTBLOCKS, STATE)
#define xof_ctx_release(STATE) PQCLEAN_MLKEM768_XOODYAK_kyber_xoodyak_ctx_release(STATE)
#define prf(OUT, OUTBYTES, KEY, NONCE) PQCLEAN_MLKEM768_XOODYAK_kyber_xoodyak_prf(OUT, OUTBYTES, KEY, NONCE)
#define rkprf(OUT, KEY, INPUT) PQCLEAN_MLKEM768_XOODYAK_kyber_xoodyak_rkprf(OUT, KEY, INPUT)
